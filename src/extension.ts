  ...
  const disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        ...
        const excludeStr =
          (cfg.get('exclude') as string) || '**/node_modules/**';
        ...
        console.log(
          '[params-to-object] includePatterns:',
          includePatterns,
          'excludePatterns:',
          excludePatterns,
          'found files:',
        ...
        if (!targetFunction) {
          ...
          for (const v of vars) {
            ...
            if (
              ...
              init.getStart() <= cursorOffset &&
              cursorOffset <= init.getEnd()
            ) {
              targetFunction = init;
              break;
            }
        ...
        if (!params || params.length === 0) {
          void vscode.window.showInformationMessage(
            'Function has zero parameters — nothing to convert.'
          );
          return;
        }
        ...
        if (
          params.length === 1 &&
          params[0].getName &&
          params[0].getName() === objectParamName
        ) {
          void vscode.window.showInformationMessage(
            'Function already uses object parameter — nothing to do.'
          );
          return;
        }
        ...
        const targetSym =
          targetFunction.getSymbol && targetFunction.getSymbol();
        ...
        if (!resolvedTarget) {
          void vscode.window.showInformationMessage(
            'Cannot resolve symbol for the selected function — aborting.'
          );
          return;
        }
        ...
        for (const sf of files) {
          ...
          if (
            sfPath &&
            sfPath.indexOf(path.sep + 'node_modules' + path.sep) >= 0
          )
            continue;
          ...
          for (const call of calls) {
            ...
            if (looksLikeCall) {
              try {
                ...
                console.log('[params-to-object] candidate call:', {
                  file: sf.getFilePath(),
                  exprText,
                  args: dbgArgs,
                ...
                console.log(
                  '[params-to-object] candidate call (could not read args) ',
                  {
                    file: sf.getFilePath(),
                    exprText,
                    resolvedCalled: !!resolvedCalled,
              ...
              try {
                ...
                if (fnId === callId) {
                  ...
                  if (
                    ...
                    console.log(
                      '[params-to-object] skipping already-object call at',
                      sf.getFilePath(),
                      'text:',
                      argsText[0]
                    );
        ...
        if (fatalConflict) {
          void vscode.window.showInformationMessage(
            `Cannot process: found call resolved to a different symbol in ${fatalConflict.file}`
          );
          return;
        }
        ...
        console.log(
          '[params-to-object] confirmed call count:',
          confirmed.length,
          'fuzzy count:',
          fuzzy.length
        );
        try {
          console.log(
            '[params-to-object] confirmed details:',
            JSON.stringify(safeSerial(confirmed), null, 2)
          );
          console.log(
            '[params-to-object] fuzzy details:',
            JSON.stringify(safeSerial(fuzzy), null, 2)
          );
        } catch (e) {
          console.log(
            '[params-to-object] confirmed examples:',
        ...
        if (fuzzy.length === 0) {
          ...
          const paramTypeText = `{ ${paramNames
            .map((n, i) => `${n}: ${paramTypes[i] || 'any'}`)
            .join('; ')} }`;
          try {
            ...
            console.log(
              '[params-to-object] error applying function text edit',
              e
            );
          }
          ...
          void vscode.window.showInformationMessage(
            `Converted ${confirmed.length} call(s) and updated function.`
          );
          return;
        }
        ...
        for (const c of allCandidates) {
          if (
            ...
            editAll.replace(
              uri,
              new vscode.Range(startP, endP),
              buildReplacementAll(c.exprText, c.argsText)
            );
        ...
        const paramTypeText2 = `{ ${paramNames
          .map((n, i) => `${n}: ${paramTypes2[i] || 'any'}`)
          .join('; ')} }`;
        try {
          ...
          const startReplace2 =
            idx2 >= 0 ? doc2.positionAt(idx2) : doc2.positionAt(targetStart);
          const endReplace2 =
            idx2 >= 0
              ? doc2.positionAt(idx2 + originalFunctionText.length)
              : doc2.positionAt(targetEnd);
          const edit3 = new vscode.WorkspaceEdit();
          edit3.replace(
            uri2,
            new vscode.Range(startReplace2, endReplace2),
            newFnText2
          );
          const ok3 = await vscode.workspace.applyEdit(edit3);
          console.log(
            '[params-to-object] applied function text edit (all):',
            ok3
          );
          await doc2.save();
          ...
          console.log(
            '[params-to-object] error applying function text edit (all)',
            e
          );
        }
  ...
  const checkDisposable = vscode.commands.registerCommand(
    ...
    () => {
      ...
      try {
        ...
        jsTsFiles = Array.from(new Set(jsTsFiles)).map((f) =>
        ...
        for (const f of funcDecls)
          toCheck.push({ node: f, name: f.getName ? f.getName() : null });
        ...
        for (const item of toCheck) {
          ...
          const resolvedFuncSym =
            funcSymbol && funcSymbol.getAliasedSymbol
              ? funcSymbol.getAliasedSymbol() || funcSymbol
              : funcSymbol;
          ...
          for (const sf of files) {
            ...
            for (const call of calls) {
              ...
              const resolvedCalled =
                calledSym && calledSym.getAliasedSymbol
                  ? calledSym.getAliasedSymbol() || calledSym
                  : calledSym;
              if (!resolvedFuncSym) {
                ambiguous = true;
              ...
              try {
                const fnName = resolvedFuncSym.getFullyQualifiedName
                  ? resolvedFuncSym.getFullyQualifiedName()
                  : resolvedFuncSym.getEscapedName &&
                    resolvedFuncSym.getEscapedName();
                const callName = resolvedCalled.getFullyQualifiedName
                  ? resolvedCalled.getFullyQualifiedName()
                  : resolvedCalled.getEscapedName &&
                    resolvedCalled.getEscapedName();
                if (fnName !== callName) {
          ...
          if (ambiguous)
            results.notWorks.push({
              name,
              reason: anyCalls
                ? 'Ambiguous or unresolved calls found'
                : 'Function symbol unresolved',
            });
          else
            results.works.push({
              name,
              note: anyCalls
                ? 'All matching calls resolved'
        ...
        for (const n of results.notWorks)
          console.log('  -', n.name, '-', n.reason);

        void vscode.window.showInformationMessage(
          'Checked functions — see developer console for details.'
        );
      } catch (err) {
        console.error(err);
        void vscode.window.showErrorMessage(
          'An error occurred during check: ' + (err.message || err)
        );
      }
    }
  );
